...
import {FirebaseRoomType, GamePlayerType, RoomReadyResponseType, UserType} from '@types-dir/index';
...
export const CheckRoomSelected = (
  ...
  const {
     getGameId, hasGameId, removeGameId
  ...
  const addRoomSubscriber = async (v: RoomReadyResponseType) => {
    const {
      checkRoomActive
    } = RoomActiveSubscriber(contextsData, gameActions);
